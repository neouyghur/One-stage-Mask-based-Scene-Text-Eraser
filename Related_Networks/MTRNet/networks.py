...
class MaskPreinpaintGenerator(BaseNetwork):
    def __init__(self, in_channels=4, residual_blocks=4, preinpaint=True, maskrefine=True,
                 use_spectral_norm=True, init_weights=True):
        super(MaskPreinpaintGenerator, self).__init__()
        ...
        if maskrefine:
            ...
            self._with_maskrefine = False

        if preinpaint:
            self._with_preinpaint = True
            ...
            self.middle3 = nn.Sequential(*blocks)


            self.preinpaint_decoder = nn.Sequential(
                spectral_norm(nn.ConvTranspose2d(in_channels=256, out_channels=128, kernel_size=4,
                                                 stride=2, padding=1), use_spectral_norm),
                nn.InstanceNorm2d(128, track_running_stats=False),
                nn.ReLU(True),
    ...
    def forward(self, x):
        ...
        if self._with_preinpaint:
            x3 = self.middle3(x)
            image = self.preinpaint_decoder(x3)
